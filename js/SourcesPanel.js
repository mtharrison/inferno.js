...
WebInspector.JavaScriptBreakpointsSidebarPane.prototype = {
    _emptyElementContextMenu: function (event) {
        ...
        var breakpointActiveTitle = breakpointActive ? WebInspector.UIString(WebInspector.useLowerCaseMenuTitles() ? "Deactivate breakpoints" : "Deactivate Breakpoints") : WebInspector.UIString(WebInspector.useLowerCaseMenuTitles() ? "Activate breakpoints" : "Activate Breakpoints");
...
WebInspector.JavaScriptOutlineDialog = function (uiSourceCode, selectItemCallback) {
    ...
    this._outlineWorker.postMessage({
        ...
        params: {
            content: uiSourceCode.workingCopy()
        }
    });
}
...
WebInspector.JavaScriptSourceFrame = function (scriptsPanel, uiSourceCode) {
    ...
    this._popoverHelper = new WebInspector.ObjectPopoverHelper(this.textEditor.element, this._getPopoverAnchor.bind(this), this._resolveObjectForPopover.bind(this), this._onHidePopover.bind(this), true);
...
WebInspector.JavaScriptSourceFrame.prototype = {
    ...
    populateLineGutterContextMenu: function (contextMenu, lineNumber) {
        ...
        if (!breakpoint) {
            ...
            contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles() ? "Add conditional breakpointâ€¦" : "Add Conditional Breakpointâ€¦"), this._editBreakpointCondition.bind(this, lineNumber));
        } else {
            contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles() ? "Remove breakpoint" : "Remove Breakpoint"), breakpoint.remove.bind(breakpoint));
            contextMenu.appendItem(WebInspector.UIString(WebInspector.useLowerCaseMenuTitles() ? "Edit breakpointâ€¦" : "Edit Breakpointâ€¦"), this._editBreakpointCondition.bind(this, lineNumber, breakpoint));
...
WebInspector.WorkersSidebarPane = function () {
    ...
    this._enableWorkersCheckbox = new WebInspector.Checkbox(WebInspector.UIString("Pause on start"), "sidebar-label", WebInspector.UIString("Automatically attach to new workers and pause them. Enabling this option will force opening inspector for all new workers."));
...
WebInspector.FormatterScriptMapping.prototype = {
    ...
    _performUISourceCodeScriptFormatting: function (uiSourceCode, callback) {
        ...
        function innerCallback(formattedContent, formatterMapping) {
            ...
            var formattedUISourceCode = (this._workspace.uiSourceCode(this._projectDelegate.id(), path));
...
WebInspector.InplaceFormatterEditorAction.prototype = {
    ...
    _isFormattable: function (uiSourceCode) {
        if (!uiSourceCode)
            return false;
        return uiSourceCode.contentType() === WebInspector.resourceTypes.Stylesheet || uiSourceCode.project().type() === WebInspector.projectTypes.Snippets;
    },
...
WebInspector.SourcesView.prototype = {
    ...
    _editorSelected: function (event) {
        var uiSourceCode = (event.data.currentFile);
...
WebInspector.SourcesPanel.prototype = {
    ...
    _updateDebuggerButtons: function () {
        if (this._paused) {
            ...
            this._pauseButton.setLongClickOptionsEnabled((function () {
                return [this._longResumeButton]
            }).bind(this));
